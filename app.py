...
def generate_3d_viewer_html(sdf_data, molecule_name, display_style='stick', component_width=600, component_height=450):
    ...
    try:
        # Set py3Dmol viewer width slightly less than the Streamlit component width to give some margin
        internal_viewer_width = component_width - 20 
        internal_viewer_height = component_height - 20 # Also adjust height if needed, or keep as passed
        ...
        viewer.zoomTo()
        # viewer.zoom(0.9) # Optional: zoom out slightly if molecule still feels too close to edges
...
if st.session_state.last_search_type == "alkane":
    ...
    # Only create tabs if there's something to show from alkane search
    ...
    else: # Handle case where alkane search yielded no results but was the last search type
        if st.session_state.alkane_molecule_searched: # If a search was attempted
...
if st.session_state.last_search_type == "alkane" and tab_objects:
    with tab_gallery: # Corresponds to tab_objects[0]
    ...
    with tab_main_props: # Corresponds to tab_objects[1]
        if st.session_state.alkane_main_molecule_props:
            ...
            st.markdown("---")
        elif st.session_state.alkane_molecule_searched : # If search was done but no props (should not happen if main_compound_obj was found)
             st.info(f"No detailed properties found for the main alkane '{st.session_state.alkane_molecule_searched}'.")


    with tab_3d_isomer: # Corresponds to tab_objects[2]
        if st.session_state.selected_isomer_cid_for_3d:
            ...
            with st.spinner(f"Loading 3D structure for {st.session_state.selected_isomer_name_for_3d}..."):
                ...
                if sdf_data:
                    ...
                    st.components.v1.html(html_3d, height=470, width=600, scrolling=False) # Passed component_width
...
if st.session_state.last_search_type == "general" and st.session_state.general_molecule_data and tab_objects:
    g_data = st.session_state.general_molecule_data
    
    # Determine tab indices based on whether 2D tab was created
    props_tab_index = 0
    view2d_tab_index = 1 if g_data.get("image_2d") else -1 # -1 if no 2D tab
    view3d_tab_index = 2 if g_data.get("image_2d") else 1

    with tab_objects[props_tab_index]: # Properties Tab
    ...
    if view2d_tab_index != -1 : # If 2D tab exists
        with tab_objects[view2d_tab_index]: # 2D Structure Tab
            if g_data.get("image_2d"):
                st.subheader(f"2D Structure for: {g_data['name']}")
                st.image(g_data["image_2d"], use_container_width=True)
            else: # Should not happen if tab exists, but as a fallback
                st.info("2D image not available for this compound.")

    with tab_objects[view3d_tab_index]: # 3D View Tab
