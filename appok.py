    ...
    try:
                ...
                if c.canonical_smiles:
                    try:
                        ...
                        if mol_obj:
                            ...
                            if is_standard_hydrocarbon: # Only proceed if not disconnected
                                for atom in mol_obj.GetAtoms():
                                    ...
                                    if atom.GetIsotope() != 0:
                                        ...
                                        is_standard_hydrocarbon = False
                                        break
                                if not is_standard_hydrocarbon: continue # Skip to next compound if isotope found
                            ...
                            if is_standard_hydrocarbon:
                                for bond in mol_obj.GetBonds():
                                    if bond.GetBondType() != Chem.BondType.SINGLE:
                                        ...
                                        is_standard_hydrocarbon = False
                                        break
                                if not is_standard_hydrocarbon: continue # Skip if non-single bond found
                                
                                if Chem.GetSymmSSSR(mol_obj): # Returns a list of rings, empty if no rings
        ...
        for isomer_entry in isomers_found_raw:
            ...
            try:
                ...
                # Check for isotopes and isolated hydrogens
                if is_valid_candidate:
                    for atom in mol_iso.GetAtoms():
                        if atom.GetSymbol() == 'H' and atom.GetDegree() == 0: # Check for isolated H (like in [HH])
                ...
                if is_valid_candidate:
                    for bond in mol_iso.GetBonds():
                        if bond.GetBondType() != Chem.BondType.SINGLE:
                            ...
                            is_valid_candidate = False
                            break
                    if not is_valid_candidate: continue # Skip if non-single bond found
                ...
                if is_valid_candidate:
                    # Use non-isomeric SMILES for uniqueness check to group stereoisomers
                    ...
                    if canonical_smiles_for_uniqueness not in unique_accepted_smiles:
                        ...
                        valid_structural_alkanes_entries.append(isomer_entry) # Store the original entry
        ...
        for final_isomer_entry in valid_structural_alkanes_entries:
            smiles_to_draw = final_isomer_entry.canonical_smiles # Use the original canonical SMILES for drawing
